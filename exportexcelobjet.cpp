...
int ExcelObject::export2Excel()
{
    if(fieldList.size() <= 0)
    {
        qDebug() << "ExcelObject::export2Excel failed: No fields defined.";
        return -1;
    }
    ...
    if(!db.isValid())
    {
        qDebug() << "ExcelObject::export2Excel failed: QODBC not supported.";
        return -2;
    }
    // set the dsn string
    ...
    if(!db.open())
    {
        qDebug() << "ExcelObject::export2Excel failed: Create Excel file failed by DRIVER={Microsoft Excel Driver (*.xls)}.";
    ...
    if(!query.exec())
    {
        qDebug() << "ExcelObject::export2Excel failed: Create Excel sheet failed.";
        //db.close();
        //QSqlDatabase::removeDatabase("excelexport");
        return -4;
    }
